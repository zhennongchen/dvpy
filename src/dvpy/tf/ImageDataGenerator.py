...
class ImageDataGenerator(object):
    ...
    def zc_random_transform(self,x):
        translation,rotation,scale,matrix_raw=dv.generate_random_transform(self.augmentation_params,x.shape[:-1])
        matrix_final=dv.transform_full_matrix_offset_center(matrix_raw,x.shape[:-1])
        x_aug = dv.apply_affine_transform(x,matrix_final[:-1, :], fill_mode="constant", cval=0., order=0)
